...
#         s_t1 = torch.abs(f1).unsqueeze(1).permute(0,1,3,2)
        f2 = torch.stft(x, n_fft=1536, hop_length=480, win_length=1536, window=torch.hann_window(window_length=1536), return_complex=True)
...
class Discriminator_stft(nn.Module):
    def __init__(self, norm='spectral_norm'):
        super().__init__()
        self.model = nn.ModuleDict()
...
#         self.apply(weights_init)
    ...
    def forward(self, x):
        f = torch.stft(x, n_fft=1024, hop_length=256, win_length=1024, window=torch.hann_window(window_length=1024).cuda(), return_complex=True)
        s_t = torch.abs(f).unsqueeze(1).permute(0,1,3,2)
...
#         s_t1 = torch.abs(f1).unsqueeze(1).permute(0,1,3,2)
        f2 = torch.stft(x, n_fft=1536, hop_length=480, win_length=1536, window=torch.hann_window(window_length=1536).cuda(), return_complex=True)
#        f2 = torch.stft(x.squeeze(1), n_fft=1024, hop_length=160, win_length=1024, window=torch.hann_window(window_length=1024), return_complex=True)
        s_t2 = torch.abs(f2).unsqueeze(1).permute(0,1,3,2)
        results = []
        for key, disc in self.model.items():
