...
def LSD(pred, target, nfft=1536, hop=480):
...
def stft(x, fft_size, hop_size, win_length, window):
    ...
    return torch.sqrt(torch.clamp(real ** 2 + imag ** 2, min=1e-16)).transpose(2, 1)
...
class SpectralConvergengeLoss1(torch.nn.Module):
    """Spectral convergence loss module."""

    def __init__(self):
        """Initilize spectral convergence loss module."""
        super(SpectralConvergengeLoss1, self).__init__()
...
class LogSTFTMagnitudeLoss1(torch.nn.Module):
    """Log STFT magnitude loss module."""

    def __init__(self):
        """Initilize los STFT magnitude loss module."""
        super(LogSTFTMagnitudeLoss1, self).__init__()
...
class STFTLoss(torch.nn.Module):
    ...
    def __init__(self, fft_size=1536, shift_size=480, win_length=1536, window="hann_window"):
...
#         y_mag = self.melspec(y)
        ...
        #mag_loss = aymmetric_mse(torch.log(x_mag), torch.log(x_mag))
        mag_loss = self.log_stft_magnitude_loss(x_mag, y_mag)

        return sc_loss, mag_loss
    
class STFTLossHighFreq(torch.nn.Module):
    """STFT loss module."""

    def __init__(self, fft_size=1536, shift_size=480, win_length=1536, window="hann_window"):
        """Initialize STFT loss module."""
        super(STFTLossHighFreq, self).__init__()
        self.fft_size = fft_size
        self.shift_size = shift_size
        self.win_length = win_length
        self.window = getattr(torch, window)(win_length)
        self.spectral_convergenge_loss = SpectralConvergengeLoss()
        self.log_stft_magnitude_loss = LogSTFTMagnitudeLoss()
        #self.aymmetric_mse = aymmetric_mse()
    ...
    def forward(self, x, y):
        ...
        x_mag = stft(x, self.fft_size, self.shift_size, self.win_length, self.window)
        y_mag = stft(y, self.fft_size, self.shift_size, self.win_length, self.window)
#         x_mag[:,:,0:int(self.fft_size * 2000 / 16000)] = x_mag[:,:,0:int(self.fft_size * 2000 / 16000)]*1e-2
#         y_mag[:,:,int(self.fft_size * 2000 / 16000):] = x_mag[:,:,int(self.fft_size * 2000 / 16000):]
    
        x_high = x_mag[:,:,int(self.fft_size * 2000 / 16000):]
        y_high= y_mag[:,:,int(self.fft_size * 2000 / 16000):]

        sc_loss = self.spectral_convergenge_loss(x_high, y_high)
        #mag_loss = aymmetric_mse(torch.log(x_high.clamp(1e-8)), torch.log(y_high.clamp(1e-8)))
        mag_loss = self.log_stft_magnitude_loss(x_high, y_high)
...
class MultiResolutionSTFTLossHighFreq(torch.nn.Module):
    """Multi resolution STFT loss module."""

    def __init__(self,
                 fft_sizes=[1024, 2048, 3072],
                 hop_sizes=[256, 512, 768],
                 win_lengths=[600, 1200, 240],
                 window="hann_window"):
        """Initialize Multi resolution STFT loss module.
        Args:
            ...
            window (str): Window function type.
        """
        super(MultiResolutionSTFTLossHighFreq, self).__init__()
        assert len(fft_sizes) == len(hop_sizes) == len(win_lengths)
        self.stft_losses = torch.nn.ModuleList()
        self.fft_sizes = fft_sizes
        for fs, ss, wl in zip(fft_sizes, hop_sizes, win_lengths):
            self.stft_losses += [STFTLossHighFreq(fs, ss, wl, window)]
...
class MultiResolutionSTFTLoss2(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.fft_sizes = fft_sizes
        for fs, ss, wl in zip(fft_sizes, hop_sizes, win_lengths):
            self.stft_losses += [STFTLoss1(fs, ss, wl, window)]
